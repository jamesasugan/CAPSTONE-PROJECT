document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderMonth() {
        ...
        let headerHtml = '<div class="grid grid-cols-7 divide-x divide-y border text-black border-gray-500">';
        ...
        let html = '<div class="grid grid-cols-7 divide-x divide-y border border-gray-500">';
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            let isToday = today.getDate() === day && today.getMonth() === currentMonth && today.getFullYear() === currentYear;
            cls += isToday ? " text-white bg-blue-500" : " text-black";
            let info = schedules[currentDate] ? `<div class="text-xs text-blue-700 mt-1 overflow-hidden whitespace-nowrap text-overflow-ellipsis">${schedules[currentDate].availability}</div>` : '';
            cls += schedules[currentDate] ? " bg-yellow-200" : ""; // Change background if there is a schedule
        ...
        document.querySelectorAll('#calendar .cursor-pointer').forEach(cell => {
            cell.addEventListener('click', function() {
                ...
                if (schedule) {
                    document.getElementById('modalTitle').textContent = `Schedule for ${date}`;
                    document.getElementById('modalContent').textContent = schedule.availability;
                    document.getElementById('modalContent').innerHTML += `<p class='text-sm text-gray-500 mt-2'>${schedule.extraInfo}</p>`; // Add extra info
